...
func TestPosterHex(t *testing.T) {
	for _, tC := range testCases {
		t.Run(tC.desc, func(t *testing.T) {
			...
			hex, err := order.PosterHex()
			require.NoError(t, err, "Expected no error generating poster hex")

			require.EqualValues(t, tC.bytes, hex, "Expected hex bytes to match")
		})
	}
}
