...
describe("KosuToken", () => {
    ...
    describe("bonding", () => {
            it("should have expected initial output with parameterization", async () => {
                ...
                TestValues.oneEther.times(30).toString().should.eq(finalBalance.toString());
                TestValues.oneEther.times(30).toString().should.eq(finalSupply.toString());
            ...
            describe("fallback", () => {
                    it("should generate tokens with fallback function", async () => {
                        ...
                        startingBalance.plus(estimate).toString().should.eq(finalBalance.toString());
                        startingSupply.plus(estimate).toString().should.eq(finalSupply.toString());
                        startingEther.plus(TestValues.oneEther).toString().should.eq(endingEther.toString());
                    });
            });
            ...
            describe("releaseTokens", () => {
                    it("should sell tokens on the curve", async () => {
                        ...
                        startingBalance.minus(TestValues.oneEther).toString().should.eq(finalBalance.toString(), "balance");
                        startingSupply.minus(TestValues.oneEther).toString().should.eq(finalSupply.toString(), "supply");
                        startingEther.minus(estimate).toString().should.eq(endingEther.toString(), "ether");
                    });
