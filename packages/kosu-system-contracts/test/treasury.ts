...
describe("Treasury", async () => {
  ...
  before(async () => {
    ...
    await kosuToken.transfer.sendTransactionAsync(accounts[1], 100);
    await kosuToken.approve.sendTransactionAsync(treasury.address, 50, { from: accounts[1] });

    // Account 2 holds 100 digm, 50 approved
    await kosuToken.transfer.sendTransactionAsync(accounts[2], 100);
    await kosuToken.approve.sendTransactionAsync(treasury.address, 50, { from: accounts[2] });

    // Account 3 has 100 digm
    await kosuToken.transfer.sendTransactionAsync(accounts[3], 100);
    await kosuToken.approve.sendTransactionAsync(treasury.address, 100, { from: accounts[3] });
  });
  ...
  describe("contractDeposit", () => {
    it("should pull tokens from the given accounts and track the balance.", async () => {
      await treasury.contractDeposit.sendTransactionAsync(accounts[1], 50, { from: accounts[5] }).should.eventually.be.fulfilled;

      await treasury.currentBalance.callAsync(accounts[1]).then(x => x.toString()).should.eventually.eq("50");
    });

    it("should fail on insufficient token approval.", async () => {
        await treasury.contractDeposit.sendTransactionAsync(accounts[1], 51, { from: accounts[5] }).should.eventually.be.rejected;
    });

    it("should fail on insufficient token balance.", async () => {
      await kosuToken.approve.sendTransactionAsync(treasury.address, 100000, { from: accounts[1] });
      await treasury.contractDeposit.sendTransactionAsync(accounts[1], 101, { from: accounts[5] }).should.eventually.be.rejected;
    });
  });

  describe("contractWithdraw", () => {
    it("should reduce the balance by the provided value.", async () => {
      await treasury.contractDeposit.sendTransactionAsync(accounts[3], "100", { from: accounts[5] });
      await treasury.contractWithdraw.sendTransactionAsync(accounts[3], 75, { from: accounts[5] }).should.eventually.be.fulfilled;
    ...
    it("should fail on over withdraw.", async () => {
      await treasury.contractWithdraw.sendTransactionAsync(accounts[3], 101, { from: accounts[5] }).should.eventually.be.rejected;
    });
  });
  ...
  describe("updateBalance", () => {
    it("should make required changes for balance to end at desired value.", async () => {
      await kosuToken.approve.sendTransactionAsync(treasury.address, 100000, { from: accounts[3] });
      await treasury.contractDeposit.sendTransactionAsync(accounts[3], "100", { from: accounts[5] });

      await treasury.currentBalance.callAsync(accounts[3]).then(x => x.toString()).should.eventually.eq("100");

      await treasury.updateBalance.sendTransactionAsync(accounts[3], 73, { from: accounts[5] });

      await treasury.currentBalance.callAsync(accounts[3]).then(x => x.toString()).should.eventually.eq("73");

      await treasury.updateBalance.sendTransactionAsync(accounts[3], 80, { from: accounts[5] });
    ...
    it("should fail on insufficient token approval.", async () => {
      await treasury.updateBalance.sendTransactionAsync(accounts[1], 51, { from: accounts[5] }).should.eventually.be.rejected;
    });

    it("should fail on insufficient token balance.", async () => {
      await kosuToken.approve.sendTransactionAsync(treasury.address, 100000, { from: accounts[1] });

      await treasury.updateBalance.sendTransactionAsync(accounts[1], 101, { from: accounts[5] }).should.eventually.be.rejected;
    });
  });
  ...
  describe("adjustBalance", () => {
    it("should change the balance by the provided value", async () => {
      ...
      await treasury.adjustBalance.sendTransactionAsync(accounts[3], -27, { from: accounts[5] });
      await treasury.currentBalance.callAsync(accounts[3]).then(x => x.toString()).should.eventually.eq("73");

      // should handle 0
      await treasury.adjustBalance.sendTransactionAsync(accounts[3], 0, { from: accounts[5] });
      await treasury.currentBalance.callAsync(accounts[3]).then(x => x.toString()).should.eventually.eq("73");

      // should handle positive change
      await treasury.adjustBalance.sendTransactionAsync(accounts[3], 7, { from: accounts[5] });
      await treasury.currentBalance.callAsync(accounts[3]).then(x => x.toString()).should.eventually.eq("80");
    ...
    it("should fail on over withdraw.", async () => {
      await treasury.adjustBalance.sendTransactionAsync(accounts[3], -101, { from: accounts[5] }).should.eventually.be.rejected;
    });

    it("should fail on insufficient token approval.", async () => {
        await kosuToken.approve.sendTransactionAsync(treasury.address, 0, { from: accounts[3] });

        await treasury.adjustBalance.sendTransactionAsync(accounts[3], 1, { from: accounts[5] }).should.eventually.be.rejected;
    });
  ...
  describe("Authorizeable", () => {
    describe("isAuthorized", () => {
      it("should allow an added account to call protected methods", async () => {
        ...
        await treasury.contractDeposit.sendTransactionAsync(accounts[1], 50, { from: accounts[6] }).should.eventually.be.rejected;

        await auth.authorizeAddress.sendTransactionAsync(accounts[6]).then(txHash => web3Wrapper.awaitTransactionSuccessAsync(txHash));

        await kosuToken.allowance.callAsync(accounts[1], treasury.address).then(x => x.toString()).should.eventually.eq("50");
        await kosuToken.balanceOf.callAsync(accounts[1]).then(x => x.toString()).should.eventually.eq("100");
        await treasury.contractDeposit.sendTransactionAsync(accounts[1], 50, { from: accounts[6] }).should.eventually.be.fulfilled;
      });
    });
  });
});
