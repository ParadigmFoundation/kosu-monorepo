...
before(async () => {
  ...
  global.kosuConnect = new Connect({ provider: web3.currentProvider, networkId: await web3.eth.net.getId() });
...
it('should have the version set', () => {
  assert.equal(require('../package').version, kosuConnect.version);
  assert.equal(require('../package').version, Connect.version);
});