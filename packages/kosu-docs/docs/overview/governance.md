...
There is also a specified `maxRewardRate` which is the maximum amount of KOSU a validator can mint per period. The `maxRewardRate` is a function equal to the square root of the sum of total reward rates for all outstanding validator listings.
