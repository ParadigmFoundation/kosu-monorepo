...
func NewWitnessTestSuite(w witness.Provider) *WitnessTestSuite {
...
func (suite *WitnessTestSuite) testHandleBlocks(t *testing.T, w witness.Provider) {
	...
	err = w.HandleBlocks(ctx, func(blk *witness.Block) {
		defer cancel()
...
func (suite *WitnessTestSuite) testHandleEvents(t *testing.T, w witness.Provider) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	var events []*witness.Event
	err := w.HandleEvents(ctx, func(e *witness.Event) {
